...
library LockDataLibrary {
    ...
    function pop() internal {
        ...
        console.log("pop", sentinel.length);
        assembly {
            // update the length at the sentinel
            sstore(sentinelSlot, sentinel)
        }
    ...
    function getActiveLock() internal view returns (address locker) {
        IPoolManager.LockSentinel memory sentinel = getLockSentinel();
        console.log("getActiveLock", sentinel.length);
        return getLock(sentinel.length - 1);
    }

    function getLockSentinel()
        internal
        view
        returns (IPoolManager.LockSentinel memory sentinel)
    {
        uint256 sentinelSlot = SENTINEL;
        assembly {
